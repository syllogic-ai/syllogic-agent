...
class TestGetDataFromFile:
    ...
    def test_get_data_csv_success(self, mock_requests, mock_supabase, sample_file_data, sample_csv_content):
    ...
    def test_get_data_missing_env_vars(self, mock_supabase, sample_file_data):
        ...
        with pytest.raises(Exception, match="SUPABASE_STORAGE_BASE_URL or SUPABASE_URL environment variable not set"):
            get_data_from_file(mock_supabase, "file-123")
...
class TestGetSampleFromFile:
    ...
    @patch('actions.dashboard.get_data_from_file')
    def test_get_sample_default_rows(self, mock_get_data, mock_supabase, sample_dataframe):
    ...
    @patch('actions.dashboard.get_data_from_file')
    def test_get_sample_specific_rows(self, mock_get_data, mock_supabase, sample_dataframe):
...
class TestCreateWidget:
    ...
    @patch('uuid.uuid4')
    def test_create_widget_success(self, mock_uuid, mock_supabase):
    ...
    def test_create_widget_with_optional_fields(self, mock_supabase):
    ...
    def test_create_widget_insert_fails(self, mock_supabase):
        ...
        with pytest.raises(Exception, match="Failed to create widget"):
            create_widget(mock_supabase, "dashboard-789", "Test", "chart", {})
...
class TestUpdateWidget:
    ...
    def test_update_widget_not_found(self, mock_supabase):
        ...
        result = update_widget(mock_supabase, "widget-123", title="Updated")
        
        assert result == {}
        
    def test_update_widget_invalid_fields_filtered(self, mock_supabase, sample_widget_data):
...
class TestGetWidgetSpecs:
    ...
    async def test_get_widget_specs_success(self, mock_supabase, sample_widget_data):
        ...
        assert result == sample_widget_data
        assert result["id"] == "widget-123"
        assert result["title"] == "Test Widget"
...
class TestGetWidgetsFromDashboardId:
    ...
    def test_get_widgets_success(self, mock_supabase, sample_widget_data):
        ...
        mock_supabase.table().select().eq.assert_called_with("dashboard_id", "dashboard-123")
